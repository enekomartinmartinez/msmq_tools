import numpy as np
import xarray as xr
from msmq_tools.opends import load_main
from msmq_tools.breakds importbreakds
...
def mask_1file(filename, savename, latname, lonname, ind, Nb):
    ...
    dx = np.sqrt((dlon[1]*111000*np.cos(np.deg2rad(lats)))**2
                 + (dlats[1]*111000)**2)
    dy = np.sqrt((dlon[0]*111000*np.cos(np.deg2rad(lats)))**2
                 + (dlats[0]*111000)**2)
    ...
    if Nb is None:
        ...
        # Save splitted data
        breakds(filename, ['mask'], latname, lonname,
